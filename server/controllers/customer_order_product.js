...
async function createOrderProduct(request, response) {
    try {
        ...
        const corder = await prisma.customerOrderProduct.create({
            data: {
                customerOrderId,
                productId,
                quantity
...
async function updateProductOrder(request, response) {
    try {
        ...
        const existingOrder = await prisma.customerOrderProduct.findUnique({
            where: {
                id: id
            }
        });
        ...
        const updatedOrder = await prisma.customerOrderProduct.update({
            where: {
                id: existingOrder.id
            },
            data: {
...
async function deleteProductOrder(request, response) {
    try {
        const { id } = request.params;
        await prisma.customerOrderProduct.deleteMany({
            where: {
                customerOrderId: id
            }
        });
...
async function getProductOrder(request, response) {
    const { id } = request.params;
    const order = await prisma.customerOrderProduct.findMany({
        where: {
...
async function getAllProductOrders(request, response) {
    try {
        ...
        const productOrders = await prisma.customerOrderProduct.findMany({
            select: {
                productId: true,
                quantity: true,
                customerOrder: {
