...
const CheckoutPage = () => {
  ...
  const makePurchase = async () => {
    if (
      ...
      const response = fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/orders`, {
        method: "POST",
        headers: {
  ...
  const addOrderProduct = async (
    ...
    const response = await fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/order-products`, {
      method: "POST", // or 'PUT'
      headers: {
        "Content-Type": "application/json",
      },
