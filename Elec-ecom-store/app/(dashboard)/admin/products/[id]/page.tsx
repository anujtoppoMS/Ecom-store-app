...
const DashboardProductDetails = ({
  ...
  const deleteProduct = async () => {
    ...
    fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/products/${id}`, requestOptions)
      .then((response) => {
  ...
  const uploadFile = async (file: any) => {
    ...
    try {
      const response = await fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/main-image`, {
        method: "POST",
        body: formData,
      });
  ...
  const fetchProductData = async () => {
    fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/products/${id}`)
      .then((res) => {
        return res.json();
      })
      .then((data) => {
    ...
    const imagesData = await fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/images/${id}`, {
      cache: "no-store",
    });
  ...
  const fetchCategories = async () => {
    fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/categories`)
      .then((res) => {
        return res.json();
      })
      .then((data) => {
