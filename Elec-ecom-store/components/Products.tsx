...
const Products = async ({ slug }: any) => {
 ...
 // If out of stock checkbox is checked, stockMode is "lt"
  ...
  const data = await fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/products?filters[price][$lte]=${slug?.searchParams?.price || 3000}&filters[rating][$gte]=${
  ...
  /*
    ...
    `http://backend-api.ecom-dev.svc.cluster.local.ecom-dev.svc.cluster.local:1337/api/products?populate=*&filters[price][$lte]=${
