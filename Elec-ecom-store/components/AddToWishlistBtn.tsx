...
const AddToWishlistBtn = ({ product, slug }: AddToWishlistBtnProps) => {
  ...
  const addToWishlistFun = async () => {
    ...
    if (session?.user?.email) {
      ...
      fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/users/email/${session?.user?.email}`, {
        cache: "no-store",
      })
        .then((response) => response.json())
        .then((data) =>
          fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/wishlist`, {
            method: "POST",
            headers: {
  ...
  const removeFromWishlistFun = async () => {
    if (session?.user?.email) {
      ...
      fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/users/email/${session?.user?.email}`, {
        ...
        .then((response) => response.json())
        .then((data) => {
          return fetch(`${`http://${process.env.BACKEND_API_SERVICE_HOST}:${process.env.BACKEND_API_SERVICE_PORT}`}/api/wishlist/${data?.id}/${product?.id}`,
            {
              method: "DELETE",
            }
          );
